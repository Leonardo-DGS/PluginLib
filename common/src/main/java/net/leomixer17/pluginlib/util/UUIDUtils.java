...
public final class UUIDUtils {
    ...
    public static UUID getOnlineUUID(String username)
    {
        try
        {
            final URLConnection conn = new URL("https://api.mojang.com/users/profiles/minecraft/" + username).openConnection();
            final JsonReader reader = new JsonReader(new InputStreamReader(conn.getInputStream()));
            reader.beginObject();
            reader.skipValue();
            final UUID uuid = fromString(reader.nextString());
            reader.close();
            return uuid;
        }
        catch (IOException e)
    ...
    public static List<String> getNameHistory(UUID uuid)
    {
        ...
        try
        {
            final URLConnection conn = new URL("https://api.mojang.com/user/profiles/" + trimUUID(uuid) + "/names").openConnection();
            final JsonReader reader = new JsonReader(new InputStreamReader(conn.getInputStream()));
            reader.beginArray();
    ...
    public static UUID fromString(String uuid)
    {
        ...
        final StringBuilder builder = new StringBuilder(uuid.trim());
        builder.insert(20, "-");
        builder.insert(16, "-");
        builder.insert(12, "-");
        builder.insert(8, "-");
